...
public:
    ...
    static void setPyramid(const Mat &img, const int levelsNum, const float scaleFactor,vector<Mat> &imagePyramid);
    ...
    static bool getCameraRT(const Mat &KMatrix,vector<Point3f> &P3D,vector<Point2f> &P2D,Mat &R,Mat &t,bool isTracked);
	void getOtherPoints();
	Mat_<float> TVector;
